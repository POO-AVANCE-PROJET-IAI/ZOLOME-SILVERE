...
from zolome_medecins.models import Medecin
from zolome_consultations.models import Consultation
from zolome_factures.models import Facture
...
def profil(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('zolome_utilisateurs:profil')
    else:
        form = UtilisateurProfilForm(instance=request.user)

    return render(request, 'zolome_utilisateurs/profil.html', {
        'title': 'Mon Profil',
        'user': request.user,
        'form': form
    })
...
def view_login_attempts(request):
    ...
    return render(request, 'zolome_utilisateurs/login_attempts.html', context)
...
def unlock_account(request, username):
    ...
    return redirect('zolome_utilisateurs:login_attempts')
